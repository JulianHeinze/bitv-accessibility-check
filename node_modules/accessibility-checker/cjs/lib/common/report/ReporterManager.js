...
const ACReporterCSV_js_1 = require("./ACReporterCSV.js");
const ACReporterHTML_js_1 = require("./ACReporterHTML.js");
const ACReporterJSON_js_1 = require("./ACReporterJSON.js");
const ACReporterXLSX_js_1 = require("./ACReporterXLSX.js");
const IRule_js_1 = require("../engine/IRule.js");
const ACReporterMetrics_js_1 = require("./ACReporterMetrics.js");
;
/**
...
class ReporterManager {
    ...
    static initialize(config, absAPI, rulesets) {
        ...
        if (config.perfMetrics) {
            ReporterManager.reporters.push(new ACReporterMetrics_js_1.ACReporterMetrics(config.toolName, config.policies));
        }
        if (!config.outputFormat.includes("disable")) {
            if (config.outputFormat.includes("json")) {
                ReporterManager.reporters.push(new ACReporterJSON_js_1.ACReporterJSON());
            }
            if (config.outputFormat.includes("html")) {
                ReporterManager.reporters.push(new ACReporterHTML_js_1.ACReporterHTML());
            }
            if (config.outputFormat.includes("csv")) {
                ReporterManager.reporters.push(new ACReporterCSV_js_1.ACReporterCSV());
            }
            if (config.outputFormat.includes("xlsx")) {
                ReporterManager.reporters.push(new ACReporterXLSX_js_1.ACReporterXLSX());
            }
        }
    }
    ...
    static generateSummaries(endReport) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const reporter of ReporterManager.reporters) {
                try {
                    let summaryInfo = yield reporter.generateSummary(ReporterManager.config, ReporterManager.rulesets, endReport || new Date().getTime(), ReporterManager.reports);
                    if (summaryInfo) {
    ...
    static valueToLevel(reportValue) {
        ...
        if (reportValue[1] === "PASS") {
            reportLevel = IConfig_js_1.eRuleLevel.pass;
        }
        else if ((reportValue[0] === "VIOLATION" || reportValue[0] === "RECOMMENDATION") && reportValue[1] === "MANUAL") {
            reportLevel = IConfig_js_1.eRuleLevel.manual;
        }
        else if (reportValue[0] === "VIOLATION") {
            if (reportValue[1] === "FAIL") {
                reportLevel = IConfig_js_1.eRuleLevel.violation;
            }
            else if (reportValue[1] === "POTENTIAL") {
                reportLevel = IConfig_js_1.eRuleLevel.potentialviolation;
            }
        }
        else if (reportValue[0] === "RECOMMENDATION") {
            if (reportValue[1] === "FAIL") {
                reportLevel = IConfig_js_1.eRuleLevel.recommendation;
            }
            else if (reportValue[1] === "POTENTIAL") {
                reportLevel = IConfig_js_1.eRuleLevel.potentialrecommendation;
            }
        }
        return reportLevel;
    }
    static filterReport(engineResult, scanLabel) {
        ...
        retVal.results = retVal.results.map(pageResult => {
            ...
            let ignored = false;
            if (pageResult.value[1] !== IRule_js_1.eRuleConfidence.PASS && pageResult.path.dom in ignoreLookup && pageResult.ruleId in ignoreLookup[pageResult.path.dom] && ignoreLookup[pageResult.path.dom][pageResult.ruleId][pageResult.reasonId]) {
                ignored = true;
            }
            return Object.assign(Object.assign({}, pageResult), { ignored, level: reportLevel });
        });
        retVal.summary = {
    ...
    static addCounts(engineReport) {
        let counts = Object.assign({ ignored: 0, elements: 0, elementsViolation: 0, elementsViolationReview: 0 }, engineReport.summary.counts);
        ...
        for (const issue of engineReport.results) {
            ...
            if (issue.ignored) {
                ...
                if (issue.level === IConfig_js_1.eRuleLevel.violation) {
...
ReporterManager.returnReporter = new ACReporterJSON_js_1.ACReporterJSON();
;
